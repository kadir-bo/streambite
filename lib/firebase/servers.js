...
export async function joinServerByCode(inviteCode, userId) {
  ...
  if (maxUses && uses >= maxUses) throw new Error('Diese Einladung ist abgelaufen')

  const memberSnap = await getDoc(doc(db, 'servers', serverId, 'members', userId))
...
export function subscribeToUserServers(serverIds, callback) {
  ...
  function emit() {
    callback(serverIds.filter((id) => servers.has(id)).map((id) => servers.get(id)))
  }
  ...
  function attach(serverId, retriesLeft = 3) {
    const unsub = onSnapshot(
      ...
      (snap) => {
        if (snap.exists()) servers.set(serverId, { id: snap.id, ...snap.data() })
...
export function subscribeToServerChannels(serverId, onChannels, onCategories) {
  ...
  const unsubChannels = onSnapshot(
    ...
    (snap) => { if (!cancelled) onChannels(snap.docs.map((d) => ({ id: d.id, ...d.data() }))) },
    (err) => {
      ...
      console.error(`[listener] servers/${serverId}/channels:`, err.code, err.message)
    },
  )

  const unsubCats = onSnapshot(
    query(collection(db, 'servers', serverId, 'categories'), orderBy('position')),
    (snap) => { if (!cancelled) onCategories(snap.docs.map((d) => ({ id: d.id, ...d.data() }))) },
    (err) => {
      if (err.code === 'permission-denied') {
        cancelled = true
        unsubChannels()
      ...
      console.error(`[listener] servers/${serverId}/categories:`, err.code, err.message)
    },
  )
...
export function subscribeToServerMembers(serverId, memberIds, callback) {
  ...
  const unsubscribers = ids.flatMap((uid) => [
    onSnapshot(
      doc(db, 'users', uid),
      (snap) => {
        if (snap.exists()) profiles.set(uid, { id: snap.id, ...snap.data() })
        emit()
      },
    ...
    onSnapshot(
      ...
      (snap) => {
        if (snap.exists()) memberMeta.set(uid, snap.data())
        emit()
      },
      (err) => console.error(`[listener] servers/${serverId}/members/${uid}:`, err.code, err.message)
    ),
  ])
...
export async function deleteChannel(serverId, channelId) {
  ...
  const sameTypeSnap = await getDocs(
    query(collection(db, 'servers', serverId, 'channels'), where('type', '==', type))
  )
...
export async function joinVoicePresence(serverId, channelId, uid, profile) {
  const db = getDB()
  await setDoc(doc(db, 'voicePresence', voicePresenceId(serverId, channelId, uid)), {
    serverId,
    channelId,
    uid,
...
export async function leaveVoicePresence(serverId, channelId, uid) {
  const db = getDB()
  await deleteDoc(doc(db, 'voicePresence', voicePresenceId(serverId, channelId, uid))).catch(() => {})
}
...
export function subscribeToServerVoicePresence(serverId, callback) {
  ...
  return onSnapshot(
    ...
    (err) => {
      if (err.code !== 'permission-denied') {
        console.error(`[listener] voicePresence (server ${serverId}):`, err.code, err.message)
      }
      callback({})
    }
  )
...
export async function declineServerInvite(uid, serverId) {
  ...
  await updateDoc(ref, { pendingInvites: existing.filter((inv) => inv.serverId !== serverId) })
}
