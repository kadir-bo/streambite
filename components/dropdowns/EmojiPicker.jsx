  ...
  return (
    ...
    >
      ...
      <div className="max-h-50 overflow-y-auto px-2 py-1.5">
        {groups.map((group) => (
          <div key={group.label}>
            <p className="text-2xs font-semibold text-zinc-500 uppercase tracking-widest px-1 pt-1.5 pb-1">
              {group.label}
            </p>
            <div className="flex flex-wrap gap-0.5">
              {group.emojis.map((emoji) => (
                <button
                  ...
                  className="size-8 rounded-sm text-lg flex items-center justify-center transition-colors duration-100 hover:bg-white/5"
                >
                  {emoji}
                </button>
              ))}
