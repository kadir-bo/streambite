...
import { MagnifyingGlass, UserPlus, Check, Warning } from '@phosphor-icons/react'
...
export default function AddFriendModal({ open, onClose }) {
  ...
  const alreadyFriend = result && result !== 'not_found' && userDoc?.friends?.includes(result.id)
  const requestPending = result && result !== 'not_found' && userDoc?.outgoingRequests?.includes(result.id)
  const isSelf = result && result !== 'not_found' && result.id === firebaseUser?.uid

  return (
    <Modal open={open} onClose={handleClose} title="Freund hinzufügen" maxWidth={440}>
      <div className="flex flex-col gap-5">
        <p className="text-sm text-(--text-muted)">
          Suche nach dem Nutzernamen mit Tag — z.B. <span className="font-(--font-mono) text-(--text-secondary)">Max#1234</span>
        </p>
        ...
        {result && result !== 'not_found' && (
          <div className="flex items-center gap-3 p-3.5 bg-(--surface-deep) rounded-(--radius-base) border border-(--border-subtle)">
            <Avatar src={result.avatarUrl} name={result.displayName} size="md" status={result.status} />
            ...
            {sent ? (
              ...
              <span className="text-xs text-(--text-muted)">Bereits befreundet</span>
            ) : requestPending ? (
              <span className="text-xs text-(--text-muted)">Anfrage ausstehend</span>
            ) : isSelf ? (
