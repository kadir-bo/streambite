...
export default function VoiceChannelView({ serverId, channel, isOwner }) {
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden bg-[#030309]">
      {/* Topbar */}
      ...
      {status === "connected" ? (
        <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 py-4 pb-28">
          ...
          {/* Screen Share */}
          {hasScreenShare && (
            <div className="flex-1 min-h-0 rounded-2xl overflow-hidden bg-[#111119]">
              {screenSharers.map((p) => (
                <ScreenShareTile
          ...
          >
            ...
            <button
              onClick={() => setInviteOpen(true)}
              className="flex flex-col items-center justify-center gap-2 rounded-2xl bg-[#111119] border border-white/5 aspect-square cursor-pointer transition-colors hover:bg-[#1c1c28]"
            >
          ...
          {hasHidden && (
            <button
              ...
              className="shrink-0 flex cursor-pointer items-center justify-center gap-1.5 self-center rounded-2xl border border-white/5 bg-[#111119] px-4 py-2 text-xs font-medium text-zinc-400 hover:bg-[#1c1c28]"
            >
              {showAll ? (
        ...
        <div className="flex flex-1 items-center justify-center px-6">
          <button
            ...
            className="flex items-center gap-2 rounded-2xl bg-[#8a38f5] px-6 py-3 text-base font-semibold text-white border-none cursor-pointer hover:bg-[#7a2de0] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <PhoneCall />
            {status === "connecting"
              ? "Verbinde…"
      ...
      {status === "connected" && (
        <div className="absolute flex items-center justify-center gap-2 bottom-4 left-0 right-0 px-4">
          <div className="flex items-center gap-2 rounded-3xl bg-[#111119] p-3">
            <button
              type="button"
              className="flex size-12 shrink-0 items-center justify-center rounded-full bg-[#1c1c28] text-white border-none cursor-pointer transition-colors hover:bg-[#252535]"
            >
              <SpeakerHigh weight="regular" className="text-xl" />
            </button>
            <button
              type="button"
              className="flex size-12 shrink-0 items-center justify-center rounded-full bg-[#7d1021] text-white border-none cursor-pointer transition-colors hover:bg-[#9a1a2d]"
            >
      ...
      {/* Connected elsewhere */}
      {connectedElsewhere && (
        <div className="absolute flex items-center gap-2 bottom-4 left-1/2 -translate-x-1/2 rounded-2xl border border-white/5 bg-[#111119] px-4 py-2 shadow-[0_8px_24px_rgba(0,0,0,0.5)]">
          ...
          <button
            ...
            className="flex items-center justify-center size-8 rounded-full bg-[#7d1021] text-white border-none cursor-pointer hover:bg-[#9a1a2d]"
          >
...
function VoiceParticipantCard({ participant, isOwner }) {
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center gap-3 rounded-2xl bg-[#111119] border aspect-square p-4 transition-all duration-200 ${
        isActiveSpeaker
