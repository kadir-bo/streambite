...
export default function VoiceChannelView({ serverId, channel, isOwner }) {
  const { connection, participants, connect } = useVoice();
  const isThisChannel =
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden bg-(--surface-base)">
      <div
        className={`flex flex-1 flex-col items-center justify-center gap-6 overflow-y-auto px-6 py-6 ${
          status === "connected" ? "pb-24" : ""
        }`}
      >
        ...
        {connection.error && (status === "error" || status === "connected") && (
          <div className="flex max-w-md flex-col items-center gap-2.5 rounded-(--radius-base) border border-(--danger) bg-(--danger-subtle) px-5 py-4 text-center">
        ...
        {status === "connected" ? (
          <>
            {participants
              .filter((p) => p.isScreenSharing && p.screenShareTrack)
              .map((p) => (
            ...
            >
              {participants.map((p) => (
                <ParticipantTile key={p.identity} participant={p} isOwner={isOwner} />
              ))}
            </div>
          </>
        ) : (
          ...
          >
            <PhoneCall size={16} />
            {status === "connecting" ? "Verbinde…" : `${channel.name} beitreten`}
          </button>
        )}
      </div>
